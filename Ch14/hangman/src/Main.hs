...
runTests :: IO ()
runTests = hspec $ do
  describe "Hangman functions" $ do
    it "fillInCharacter should fill in a guessed char" $ do
      let puzzleBefore = (Puzzle "alabama"
                                 [Nothing, Nothing, Nothing,
                                 Nothing, Nothing, Nothing, Nothing]
                                 "")
      let puzzleAfter  = (Puzzle "alabama"
                                 [Just 'a', Nothing, Just 'a', Nothing,
                                 Just 'a', Nothing, Just 'a']
                                 "a")
      fillInCharacter puzzleBefore 'a' `shouldBe` puzzleAfter

    it "handleGuess should return unchanged puzzle for repeat guess" $ do
      let puzzleBefore = (Puzzle "alabama"
                                 [Nothing, Nothing, Nothing,
                                 Nothing, Nothing, Nothing, Nothing]
                                 "c")
      puzzleAfter <- handleGuess puzzleBefore 'c'
      puzzleAfter `shouldBe` puzzleBefore

    it "handleGuess should add missed guess to list of guessed chars" $ do
      let puzzleBefore  = (Puzzle "alabama"
                                 [Nothing, Nothing, Nothing,
                                 Nothing, Nothing, Nothing, Nothing]
                                 "c")
      let expectedAfter = (Puzzle "alabama"
                                  [Nothing, Nothing, Nothing,
                                  Nothing, Nothing, Nothing, Nothing]
                                  "dc")
      puzzleAfter <- handleGuess puzzleBefore 'd'
      puzzleAfter `shouldBe` expectedAfter

    it "handleGuess should fill in correct guess" $ do
      let puzzleBefore =   (Puzzle "alabama"
                                   [Nothing, Nothing, Nothing,
                                   Nothing, Nothing, Nothing, Nothing]
                                   "")
      let expectedAfter  = (Puzzle "alabama"
                                   ...
                                   Just 'a', Nothing, Just 'a']
                                   "a")
      puzzleAfter <- handleGuess puzzleBefore 'a'
      puzzleAfter `shouldBe` expectedAfter
