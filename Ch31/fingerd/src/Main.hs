...
createDatabase = do
  ...
  where meRow :: UserRow
        meRow =
          (Null, "dkimbel", "/bin/zsh", "/Users/dkimbel",
           "Daniel Kimbel", "000-000-0000")
...
formatStr :: String -> ByteString
formatStr s = encodeUtf8 $ T.pack s
...
returnUser dbConn soc username = do
  ...
  case maybeUser of
    Nothing -> do
      let errorStr = "Couldn't find matching user\
                     \ for username: " ++ show username
      putStrLn errorStr
      sendAll soc (formatStr errorStr)
    Just user ->
