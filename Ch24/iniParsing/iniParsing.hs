...
-- The book's demonstration of why `skipEOL` is necessary;
-- without it, our second call to parseAssignmentBad would
-- immediately fail by trying to parse a newline as `some
-- letter`, so we'd only ever successfully parse a single
-- assignment and then exit.
--parseAssignmentBad :: Parser (Name, Value)
--parseAssignmentBad = do
--  name <- some letter
--  _ <- char '='
--  val <- some (noneOf "\n")
--  return (name, val)
...
-- not really related to the rest of this file, but also
-- from the book
p' :: Parser [Integer]
p' = some $ do
  i <- token (some digit)
  ...
  describe "Section Parsing" $
    it "can parse a simple section" $ do
      let m = parseByteString parseSection
          ...
          expected' = Just (Section (Header "states")
                                    states)
--      print m
      r' `shouldBe` expected'
